...
import com.chiancloud.docdive.dao.DocumentDao;
import com.chiancloud.docdive.domain.Document;
import com.chiancloud.docdive.domain.Page;
import com.chiancloud.docdive.domain.ResultInfo;
...
public class DocumentService {
	...
	public Object getDocument(String userId, String documentId) {
		...
		List<Tenant> tenants = permissionService.geTenantsByUserId(userId);
		//当前用户可以查看其所属部门（租户）的文档
		for (Tenant tenant : tenants) {
			if(tenant.getTenantId().equals(document.getTenant_id())){
				return res;
			}
		}
		info.setCode(403);
		info.setMessage("view document error: Permission denied");
		return info;
	}
	...
	public Object updateDocument(String userId, String documentId, Map<String, String> fileInfo) {
		...
		if (!permissionService.isDataAdmin(userId) && !document.getUser_id().equals(userId)) {
	...
	private String getTenantInfoByUserId(String userId, Map<String, String> tenantNames) {
		...
		String condition = "('";
		for (Tenant tenant : tenants) {
			condition += tenant.getTenantId() + "','";
			tenantNames.put(tenant.getTenantId(), tenant.getTenantName());
		}
